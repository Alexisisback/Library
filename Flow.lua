...
function convertTableToString(inputTable)
    if typeof(inputTable) ~= "table" then
        inputTable = { tostring(inputTable) }
    end
...
local old_Lnx = CoreGui:FindFirstChild('Lnx')

if old_Lnx then
    Debris:AddItem(old_Lnx, 0)
end

if not isfolder("Lnx") then
    makefolder("Lnx")
end
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('Lnx/'..file_name..'.json', flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('Lnx/'..file_name..'.json') then
                self:save(file_name, config)
        
                return
            end
        
            local flags = readfile('Lnx/'..file_name..'.json')
...
function Library.SendNotification(settings)
    ...
    InnerFrame.Size = UDim2.new(1, 0, 0, 55)
    InnerFrame.BackgroundColor3 = Color3.fromRGB(22, 28, 38)
    ...
    Gradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(28, 30, 45)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(40, 44, 66))
    }
    ...
    InnerFrame.Position = UDim2.new(1, 310, 0, 10 + NotificationContainer.Size.Y.Offset)
...
function Library:get_screen_scale()
    ...
    self._ui_scale = viewport_size_x / 1300
end
...
function Library:create_ui()
    ...
    local Lnx = Instance.new('ScreenGui')
    Lnx.ResetOnSpawn = false
    Lnx.Name = 'Lnx'
    Lnx.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    Lnx.Parent = CoreGui
    ...
    function self:load()
        ...
        for _, object in Lnx:GetDescendants() do
    ...
    function self:create_tab(title: string, icon: string)
        ...
        function TabManager:create_module(settings: any)
            ...
            if not settings.rich then
                ...
                ModuleName.Text = settings.richtext or "<font color='rgb(255,0,0)'>Lnx</font> user"
            end;
            ...
            function ModuleManager:create_button(settings: any)
                ...
                local Container = Instance.new("Frame")
                Container.Size = UDim2.new(0, 207, 0, 20)
                ...
                Button.Text = settings.title or "skibibi"
            ...
            function ModuleManager:create_text(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>Lnx</font> user" -- Default rich text
                end
                ...
                function TextManager:Set(new_settings)
                    if not new_settings.rich then
                        ...
                        Body.RichText = true
                        Body.Text = new_settings.richtext or "<font color='rgb(255,0,0)'>Lnx</font> user" -- Default rich text
                    end
                end;
